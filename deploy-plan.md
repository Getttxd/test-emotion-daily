...
2. ตั้งชื่อ เช่น `daily-emotion-deploy`
...
| 5 | `PAGES_PROJECT_NAME` | ชื่อ project ตั้งเอง ต้อง **unique ทั่วโลก** เช่น `john-daily-emotion` | ตั้งเอง |
...
`https://daily-emotion-backend.<subdomain>.workers.dev`
...
gh secret set VITE_BACKEND_URL --body "https://daily-emotion-backend.<subdomain>.workers.dev"
```
...
curl https://daily-emotion-backend.<subdomain>.workers.dev/health

# API docs (เปิดในเบราว์เซอร์)
# https://daily-emotion-backend.<subdomain>.workers.dev/docs
...
npx wrangler d1 migrations apply daily-emotion-db --remote
npm run deploy

# --- Frontend ---
cd ../frontend
pnpm install
VITE_BACKEND_URL="https://daily-emotion-backend.<subdomain>.workers.dev" pnpm build
