...
<template>
  <div>
    ...
    <VDialog v-model="dialog" max-width="480" persistent>
      <VCard :title="editingUser ? t('user.editTitle') : t('user.addTitle')">
        <VCardText>
        ...
        <VCardActions class="justify-end pa-4">
          <VBtn variant="text" @click="dialog = false">{{ t('user.cancel') }}</VBtn>
          <VBtn
            color="primary"
            :loading="isSubmitting"
            @click="submit"
          >
            {{ editingUser ? t('user.save') : t('user.create') }}
          </VBtn>
        </VCardActions>
      </VCard>
    </VDialog>
    ...
    <VDialog v-model="deleteDialog" max-width="400">
      <VCard :title="t('user.deleteTitle')">
        <VCardText>
          {{ t('user.deleteConfirm', { name: deletingUser?.name }) }}
        </VCardText>
        <VCardActions class="justify-end pa-4">
          <VBtn variant="text" @click="deleteDialog = false">{{ t('user.cancel') }}</VBtn>
          <VBtn
            color="error"
            :loading="isSubmitting"
            @click="confirmDelete"
